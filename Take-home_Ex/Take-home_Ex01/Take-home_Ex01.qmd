...
-   Overall trend that ships running on **Steam Turbine** is generally the most fuel efficient of the 3 engine types (highest nautical miles per kWh of energy).

-   From the plot, it appears that there is a possiblity of seasonal trend, but this differs across all 3 engine types.

-   For example in January 2024, Steam Turbine ships are at their lowest efficiency level (across the time period of July 2023 to June 2024) whereas for diesel ships, they are at highest efficiency level. This is indicative that other variables need to be taken into consideration when exploring fuel efficiency (i.e. what kind of routes are these ships on, load that each ship is carrying in different periods etc.)
:::
