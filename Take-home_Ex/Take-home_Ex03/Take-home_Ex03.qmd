...
```{r}
plot_rainfall_map(rainfall_map_data, 2024, "December")
```
...
### 3.3.1 IDW interpolation for total rainfall

```{r}
pacman::p_load(raster, gstat)
```

**Convert coordinates to x, y spatial points:**

```{r}
climate_rainfall3414 <- cbind(climate_rainfall3414, st_coordinates(climate_rainfall3414))
```

**Define intepolation grid and create grid of points:**

```{r}
x_rainfall <- seq(min(climate_rainfall3414$X), max(climate_rainfall3414$X), by = 20)
y_rainfall <- seq(min(climate_rainfall3414$Y), max(climate_rainfall3414$Y), by = 20)

grid_rainfall <- expand.grid(x = x_rainfall, y = y_rainfall)

grid_rainfall_sf <- st_as_sf(grid_rainfall, coords = c("x", "y"), crs = 3414)
st_crs(grid_rainfall_sf)
```

**Perform IDW Interpolation:**

```{r}
idw_rainfall <- idw(formula = Daily.Rainfall.Total..mm. ~ 1, locations = climate_rainfall3414, newdata = grid_rainfall_sf)

```

```{r}
summary(idw_rainfall)
head(idw_rainfall)
```
