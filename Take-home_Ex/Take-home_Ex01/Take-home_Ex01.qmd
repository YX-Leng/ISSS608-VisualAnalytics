...
### 4.4 Time Trend of Average Revenue and Cost
...
plot_revenue <- plot_ly(data = ship_data_Timetrend, 
        x = ~Year_Month, 
        y = ~Average_Revenue, 
        color = ~Route_Type, 
        type = 'scatter', 
...
plot_cost <- plot_ly(data = ship_data_Timetrend, 
        x = ~Year_Month, 
        y = ~Average_Cost, 
        color = ~Route_Type, 
        type = 'scatter', 
...
#| code-fold: true
# Extract Year-Month
ship_Timetrend2 <- ship_data_clean %>%
  mutate(Year_Month = as.Date(format(Date, "%Y-%m-01"))) %>%
