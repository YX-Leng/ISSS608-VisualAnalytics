...
generate_idw_rainfall <- function(data, year, month, resolution, nmax, mpsz) {
  
  # Read the shapefile inside the function
  mpsz <- st_read("data/geospatial/MP14_SUBZONE_WEB_PL.shp") %>%
    st_transform(crs = 3414)  # Transform to EPSG:3414
  
  # Filter rainfall data based on the year and month
  filtered_data <- data %>%
    filter(Year == year, Month == month)
...
plot_rainfall_idw <- function(data, year, month, resolution, nmax) {
  tmap_mode("plot")
  map <- tm_shape(data) +
...
generate_idw_temperature <- function(data, year, month, resolution, nmax, mpsz) {
...
plot_temperature_idw <- function(data, year, month, resolution, nmax) {
  tmap_mode("plot")
  map <- tm_shape(data) +
    tm_symbols(col = "var1.pred", 
               size = "var1.pred", 
...
generate_idw_windspeed <- function(data, year, month, resolution, nmax, mpsz) {
...
plot_windspeed_idw <- function(data, year, month, resolution, nmax) {
  tmap_mode("plot")
  map <- tm_shape(data) +
    tm_symbols(col = "var1.pred", 
               size = "var1.pred", 
