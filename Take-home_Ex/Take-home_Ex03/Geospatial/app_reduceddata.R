...
localmoran_i_rainfall <- function(data, year, month, k_neighbors = 2) {
  ...
  # Correcting the filter part
  filtered_data <- data %>%
    filter(Year == year, Month == month) %>%
    filter(!is.na(Total_Rainfall))
  
  # Checking if the Total_Rainfall column exists
  if (!"Total_Rainfall" %in% names(filtered_data)) {
    stop("The 'Total_Rainfall' column does not exist in the filtered data.")
  }
  
  # Convert to sf object for spatial operations
  rainfall_morandata <- st_as_sf(filtered_data, coords = c("Longitude", "Latitude"), crs = 3414)
  
  coordinates <- st_coordinates(rainfall_morandata)
  coordinates <- as.data.frame(coordinates)
  coordinates[] <- lapply(coordinates, as.numeric)
  
  # Neighbors and weight calculation
  ...
  # Add Local Moran's I to the filtered data
  filtered_data$Local_Moran_I <- local_moran_result[, 1]  
  
  # Combine the calculated Local Moran's I with original data
  rainfall_morandata$Local_Moran_I <- filtered_data$Local_Moran_I
  
  return(rainfall_morandata)
}
...
localmoran_i_temperature <- function(data, year, month, k_neighbors = 2) {
  ...
  temperature_morandata <- st_as_sf(filtered_data, coords = c("Longitude", "Latitude"), crs = 3414)
  
  coordinates <- st_coordinates(temperature_morandata)
  coordinates <- as.data.frame(coordinates)
  coordinates[] <- lapply(coordinates, as.numeric)
  ...
  temperature_morandata$Local_Moran_I <- filtered_data$Local_Moran_I
  
  return(temperature_morandata)
}
...
localmoran_i_windspeed <- function(data, year, month, k_neighbors = 2) {
  ...
  # Fixing filter chain issue and ensuring Mean_Wind_Speed is present
  filtered_data <- data %>%
    filter(Year == year, Month == month) %>%
    filter(!is.na(Mean_Wind_Speed))  # Corrected filter chain
  
  # Check if Mean_Wind_Speed column exists
  if (!"Mean_Wind_Speed" %in% names(filtered_data)) {
    stop("The 'Mean_Wind_Speed' column does not exist in the filtered data.")
  }
  
  # Convert filtered data to sf object
  windspeed_morandata <- st_as_sf(filtered_data, coords = c("Longitude", "Latitude"), crs = 3414)
  
  # Get the coordinates
  coordinates <- st_coordinates(windspeed_morandata)
  coordinates <- as.data.frame(coordinates)
  coordinates[] <- lapply(coordinates, as.numeric)
  ...
  # Variable for local Moran's I calculation
  variable <- filtered_data$Mean_Wind_Speed
  local_moran_result <- localmoran(variable, weights)
  
  # Add Local Moran's I values to filtered data
  filtered_data$Local_Moran_I <- local_moran_result[, 1]  
  
  # Add the Local Moran's I values to the sf object
  windspeed_morandata$Local_Moran_I <- filtered_data$Local_Moran_I
  
  return(windspeed_morandata)
}



# Plot function for Rainfall Moran's I
plot_rainfall_morani <- function(rainfall_morandata, year, month, k_neighbors = 2) {
  tmap_mode("view")
  
  rainfall_morani <- tm_shape(rainfall_morandata) +
    tm_symbols(
      size = "Total_Rainfall", 
      fill = "Local_Moran_I",
...
plot_temperature_morani <- function(temperature_morandata, year, month, k_neighbors = 2) {
  tmap_mode("view")
  
  temperature_morani <- tm_shape(temperature_morandata) +
    tm_symbols(
      size = "Mean_Temperature", 
      fill = "Local_Moran_I",
      size.scale = tm_scale_continuous(values.scale = 3),
...
plot_windspeed_morani <- function(windspeed_morandata, year, month, k_neighbors = 2) {
  tmap_mode("view")
  
  windspeed_morani <- tm_shape(windspeed_morandata) +
    tm_symbols(
      size = "Mean_Wind_Speed", 
      fill = "Local_Moran_I",
      size.scale = tm_scale_continuous(values.scale = 3),
...
ui <- navbarPage(
  ...
  navbarMenu(
    ...
    tabPanel("Exploratory Data Analysis",
             sidebarLayout(
               sidebarPanel(
                 ...
                 # Action button to generate the static map
    ...
    # Replace the existing Spatial Autocorrelation tab Panel with:
...
server <- function(input, output, session) {
  ...
  climate_rainfall_sf <- reactive({
    req(climate_geospatial)
    if (!inherits(climate_geospatial, "sf")) {
      stop("Data must be an sf object")
    }
    if (is.na(st_crs(climate_geospatial))) {
      climate_geospatial <- st_set_crs(climate_geospatial, 3414)
    }
    return(climate_geospatial)
  })
...
#Generate maps only when action button is clicked
  ...
  # Rainfall bubble map  
    ...
    # Rainfall Moran I bubble map  
    tryCatch({
      withProgress(message = 'Creating map...', {
        output$rainfall_morani_map <- renderTmap({
          rainfall_morandata <- localmoran_i_rainfall(
            data = climate_geospatial,
    ...
    # Temperature Moran I bubble map  
    tryCatch({
      withProgress(message = 'Creating map...', {
        output$temperature_morani_map <- renderTmap({
          temperature_morandata <- localmoran_i_temperature(
            data = climate_geospatial,
            year = input$morani_year,
            month = input$morani_month,
            k_neighbors = input$k_neighbors
    ...
    # Wind Speed Moran I bubble map  
    tryCatch({
      withProgress(message = 'Creating map...', {
        output$windspeed_morani_map <- renderTmap({
          windspeed_morandata <- localmoran_i_windspeed(
            data = climate_geospatial,
            year = input$morani_year,
            month = input$morani_month,
            k_neighbors = input$k_neighbors
  ...
  observe({
    ...
    available_months <- climate_geospatial %>%
  ...
  observe({
    available_years <- unique(year(climate_geospatial$date))
